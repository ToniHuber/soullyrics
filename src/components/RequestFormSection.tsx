...
function StepIndicator({ step }: { step: 1 | 2 }) {
  const steps: { n: 1 | 2; label: string }[] = [
    { n: 1, label: "Kontaktdaten" },
    { n: 2, label: "Projektdetails" },
  ];
  return (
        ...
        <div key={s.n} className="flex items-center gap-3 flex-1 last:flex-none">
          <div
            className={`flex items-center gap-2 text-xs font-semibold whitespace-nowrap ${
              step === s.n ? "text-gold-400" : step > s.n ? "text-white/70" : "text-white/35"
            }`}
          >
            <span
...
export function RequestFormSection() {
  ...
  const validateStep1 = (): FieldErrors => {
    ...
    if (!form.address.trim()) errs.address = "Bitte gib deine Adresse an.";
    if (!form.email.trim()) errs.email = "Bitte gib deine E-Mail-Adresse an.";
    else if (!emailRegex.test(form.email.trim())) errs.email = "Bitte gib eine gültige E-Mail-Adresse an.";
    if (!form.phone.trim()) errs.phone = "Bitte gib deine Telefonnummer an.";
    if (!form.contactMethod) errs.contactMethod = "Bitte wähle deine bevorzugte Kontaktart aus.";
    return errs;
  };

  const validateStep2 = (): FieldErrors => {
    const errs: FieldErrors = {};
    if (!form.service) errs.service = "Bitte wähle eine Leistung aus.";
    if (!form.occasion) errs.occasion = "Bitte wähle einen Anlass aus.";
    if (showOccasionOther && !form.occasionOther.trim()) errs.occasionOther = "Bitte beschreibe den Anlass.";
    if (!form.genre) errs.genre = "Bitte wähle eine Musikrichtung aus.";
    if (!form.idea.trim()) errs.idea = "Bitte beschreibe die Grundidee.";
    if (!form.wishes.trim()) errs.wishes = "Bitte gib an, was unbedingt vorkommen soll.";
    if (!form.language) errs.language = "Bitte wähle eine Sprache aus.";
    if (!form.voice) errs.voice = "Bitte wähle eine Stimmwahl aus.";
    if (!form.length) errs.length = "Bitte wähle eine gewünschte Länge aus.";
    if (!form.usage) errs.usage = "Bitte wähle die geplante Verwendung aus.";
    if (showUsageOther && !form.usageOther.trim()) errs.usageOther = "Bitte beschreibe die Verwendung.";
    if (!form.deadline) errs.deadline = "Bitte gib einen gewünschten Fertigstellungstermin an.";
    if (!form.paymentMethod) errs.paymentMethod = "Bitte wähle eine Zahlungsart aus.";
    if (!form.deliveryMethod) errs.deliveryMethod = "Bitte wähle einen Lieferweg aus.";
    if (!form.consent) errs.consent = "Bitte bestätige die Datenschutzhinweise.";
    if (!form.agbAccepted) errs.agb = "Bitte bestätige die AGB.";
    return errs;
  };
                  ...
                  <>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      <button
                        type="button"
                        onClick={goToStep1}
                        className="btn-outline !text-base sm:w-auto"
                      >
